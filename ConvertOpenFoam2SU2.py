...
#
# Update to be compatible with OpenFOAM v2212 mesh in tutorials/incompressible/simpleFoam/airFoil2D
# By Shi Jingchang
#
# Copyright 2023
# Author Shi Jingchang
# jingchangshi@gmail.com
#
# License
#
...
listLinesCellsFile = []
try:
    print("Reading of the 'cells' file")
    pathAndFileName = pathAndFileName.replace("faces", "cells")
    CellsFile = open(pathAndFileName, 'r')
    for line in CellsFile:
        listLinesCellsFile.append(line[0:-1])
    CellsFile.close()
    # Reads the number of cells
    # The first lines are skipped
    i = SkipFirstLines(listLinesCellsFile)
    nb_cells = int(listLinesCellsFile[i])
    ...
    for n in range(nb_cells):
        listCells.append([])
    listLinesCellsFile = []
    print("'cells' file read correctly\n")
except:
    print("Problem during the 'cells' file reading")
    exit(1)
...
try:
    ...
    pathAndFileName = pathAndFileName.replace("cells", "owner")
...
try:
    ...
    nb_internal_faces = int(listLinesNeighbourFile[i])
    print("number of internal faces = ", nb_internal_faces)
    i = i + 1
...
try:
    ...
    print("number of boundaries =", nb_boundaries)
    i = i + 1
    ...
    print("Boundaries =")
    n = 1
    for k in listBoundaries.keys():
        print("No. %d: Name = %s, Type = %s, NumberOfFaces = %d"%(n, k, listBoundaries[k][2], listBoundaries[k][1]))
        n = n + 1
